...
const logos = [
  ...
  { img: fltech_logo, title: 'Florida Institute of Techonology', subtitle: '' },
  { img: ksu_logo, title: 'Kennesaw State University', subtitle: '' },
  { img: uab_logo, title: 'Univeristy of Alabama at Birmingham', subtitle: '' },
  { img: uga_logo, title: 'University of Georgia', subtitle: '' },
  { img: uncc_logo, title: 'Univeristy of North Carolina at ', subtitle: '' },
...
export default function Institutions() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
        ...
        const trackWidth = track.scrollWidth / 2; // Half of total width (1 loop)
        ...
        repeat: -1,
        modifiers: {
            x: gsap.utils.unitize((x) => parseFloat(x) % trackWidth), // wrap around cleanly
        },
        });
  ...
  return (
    <section className="bg-[#F0F0F0] pt-30 pb-35 md:pt-40 md:pb-45 w-full flex flex-col items-start" ref={containerRef}>
      <div className="w-full max-w-7xl pl-4 pr-4 md:pl-8 md:pr-4">
        <h2 className="text-2xl md:text-3xl font-bold mb-12 custom-shadow text-white text-center md:text-left">Previous Participating Institutions:</h2>
      </div>
      ...
      <div className="overflow-hidden w-full relative">
        <div className="flex carousel-track w-[200%] ">
          {[...logos, ...logos].map((logo, index) => (
            <div
              key={index}
              className="w-60 aspect-square flex-shrink-0 mx-6 relative group drop-shadow-lg"
            ...
            >
              <img
                ...
                alt={logo.title}
                loading="lazy"
                className="w-full h-full object-cover transition duration-300 ease-in-out group-hover:brightness-40"
              />
              <div className="absolute inset-0 flex flex-col justify-center px-2 text-white text-center opacity-0 group-hover:opacity-100 transition duration-300 ease-in-out">
                <h3 className="text-xl font-bold drop-shadow-md custom-shadow whitespace-normal break-words">{logo.title}</h3>
                <p className="text-md drop-shadow-md custom-shadow whitespace-normal break-words">{logo.subtitle}</p>
              </div>
            </div>
          ))}
        </div>
